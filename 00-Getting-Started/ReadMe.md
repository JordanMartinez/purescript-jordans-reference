...
- People (wrongly) believe that they must know a very abstract mathematics called "Category Theory" in order to use/write PureScript or another FP language. Due to its very abstract nature, it can be difficult to grasp.
    - This "myth" is false. Similar to a non-techy person being able to use a very powerful computer to check their email without knowing how that computer works (e.g. the entire process from hardware -> boot process -> operating system -> email application), an FP programmer can use concepts that have their foundations in Category Theory without fully understanding those foundations. In short, one can produce high-quality programs in an FP language for years without fully understanding all of these Category Theory concepts.
    - Someone already familiar with FP concepts might want to learn Category Theory to learn new ways of structuring the control flow of a program, but a new learner can often get more benefit by spending their time learning something else rather than Category Theory. In short, one can learn it if they choose to, but it's not necessary, nor always worth the time investment. It might be more useful after one is already familiar with writing FP code.
